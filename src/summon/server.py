"""Summon MCP server - Analyze large file collections with AI models."""
...
async def test_api_connection(server: SummonServer):
    ...
    test_model = TEST_MODELS.get(server.provider, TEST_MODELS["google"])
...
async def main():
    ...
    if len(args) < MIN_ARGS:
        ...
        print("Usage: summon <provider> <api-key> [--test]")
        print()
        print("Providers: google, openai")
        print()
        print("Examples:")
        print("  summon google AIza...")
        print("  summon openai sk-proj-...")
        print("  summon google sk-or-v1-... --test")
    ...
    if provider not in ["google", "openai"]:
        ...
        print("Valid providers: google, openai")
        sys.exit(1)

    # Create server with stored configuration
    server = SummonServer("summon", api_key, provider)
    ...
    logger.info("Starting Summon MCP Server")
...
def run():
    """Entry point for the summon command."""
    import asyncio
