# Error Handling and Crash Recovery Analysis for Summon
...
- **No try/except in main loop**: The `server.run()` call in lines 188-199 has
...
- **Some try/except blocks**: Has basic error handling for model info (lines
  15-70)
- **Provider errors passed through**: Errors from providers are returned but not
  logged
...
Create `summon-supervisor.py`:

```python
#!/usr/bin/env python3
"""Supervisor script to auto-restart summon on crashes."""
...
def run_server(args):
    """Run the summon server and monitor it."""
    max_restarts = 5
    restart_count = 0
    ...
    while True:
        ...
        print(f"\n[{datetime.datetime.now()}] Starting summon server...")
...
- The stdio transport makes recovery challenging - when the process dies, the
  pipe breaks
